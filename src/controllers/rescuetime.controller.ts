...
export const start_focustime = (request: Request, response: Response) => {
  ...
  if (request.app.get("debug"))
    console.log(`Passing rescuetime a start session prompt with a duration of `
                + `${duration} minutes`);
  axios.post(`https://www.rescuetime.com/anapi/start_focustime?key=`
             + `${request.get('X-Api-Key')}&duration=${duration}`)
    .then((resp) => {
      if(request.app.get("debug"))
...
export const end_focustime = (request: Request, response: Response) => {
  ...
  axios.post(`https://www.rescuetime.com/anapi/end_focustime?key=`
