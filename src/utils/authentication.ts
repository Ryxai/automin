...
export const apiKeyCheck = async (request : Request) => {
  ...
  if (request.app.get("debug"))
    console.log(`Authenticating API key: default_token: ${default_token} :: hashed_apikey: ${hashed_apikey}`);
  const result = await compare(request.get("Automin-API-Key") ?? default_token, hashed_apikey);
  if (request.app.get("debug"))
